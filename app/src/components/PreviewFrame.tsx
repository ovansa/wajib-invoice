...
export default function PreviewFrame({
  ...
  useEffect(() => {
    ...
    sheetRefs.current.slice(0, pageCount).forEach((el) => el && io.observe(el));
  ...
  const fitWidth = () => {
    ...
    if (w)
      setZoom(
        clamp(
          Math.floor(((w - 8) / PAGE_WIDTH) * 100) / 100,
          MIN_ZOOM,
          MAX_ZOOM,
        ),
      );
  ...
  const zoomBy = (delta: number) =>
    setZoom((z) =>
      clamp(Math.round((z + delta) * 10) / 10, MIN_ZOOM, MAX_ZOOM),
    );
  ...
  const naturalHeight = pageCount * PAGE_HEIGHT + (pageCount - 1) * PAGE_GAP;
              ...
              >
                {nearPages.has(i) && (
                  <>
                    ...
                    >
                      <div
                        style={{ transform: `translateY(${-band.start}px)` }}
                      >
                        {children}
                      </div>
                    </div>
