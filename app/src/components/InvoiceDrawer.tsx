...
import { STATUS_META, nextStatus } from "../lib/store";
...
type Props = {
  ...
  onCycleStatus: (id: string, next: InvoiceStatus) => void;
};
              ...
              <li key={inv.id}>
                ...
                >
                  ...
                  <div className="flex flex-col items-end gap-2">
                    {/* Status pill — click to cycle Draft → Sent → Paid */}
                    <button
                      onClick={() => onCycleStatus(inv.id, nextStatus(inv.status))}
                      title="Click to change status"
                      className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[11px] font-semibold transition-colors ${meta.chip}`}
                    >
